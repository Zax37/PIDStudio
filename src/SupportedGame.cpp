...
void SupportedGame::initializeLibrary(const std::shared_ptr<AssetLibrary::TreeNode>& root) {
    root->name = gettext(name);
    std::sort(root->children.begin(), root->children.end(), sortElements);
}

std::string SupportedGame::getNames(const std::vector<std::shared_ptr<SupportedGame>>& games) {
    switch (games.size()) {
    case 0:
        return "";
    case 1:
